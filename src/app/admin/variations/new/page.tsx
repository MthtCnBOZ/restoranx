...
export default function NewVariationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Varyasyon eklenirken bir hata oluştu");
      }

      toast.success("Varyasyon başarıyla eklendi");
      router.push("/admin/variations");
    } catch (error: any) {
      console.error("Varyasyon ekleme hatası:", error);
      toast.error(error.message || "Varyasyon eklenirken bir hata oluştu");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4 md:gap-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl md:text-3xl font-bold">Yeni Varyasyon</h1>
      </div>
      ...
      <Card>
        <form onSubmit={handleSubmit}>
          <CardHeader>
            ...
            <CardDescription>
              Ürünlere eklenebilecek yeni bir varyasyon oluşturun (örn. Boyut, Hamur Tipi, vb.)
            </CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button type="submit" disabled={loading}>
              {loading ? "Kaydediliyor..." : "Varyasyon Ekle"}
            </Button>
          </CardFooter>
        </form>
      </Card>
